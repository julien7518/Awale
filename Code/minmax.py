...
def minMax(jeu: Awale, profondeur: int, alpha: int, beta: int, joueuramaximiser: bool, eval: Callable[[Awale, int], int], optiAB: bool = True) -> Tuple[Optional[int], int]:
    ...
    :param optiAB: Utilisation de l'élagage Alpha-Bêta
    :type optiAB: Optionnal[bool]
    ...
    if joueuramaximiser:
        ...
        for i in liste_coup_pos:
            ...
            copie.joue(i)
            nouveau_res = minMax(copie, profondeur-1, alpha, beta, False, eval, optiAB)[1]
    ...
    # Minimisateur
        ...
        for i in liste_coup_pos:
            copie = copy.deepcopy(jeu)
            copie.joue(i)
            nouveau_res = minMax(copie, profondeur-1, alpha, beta, True, eval, optiAB)[1]
            if nouveau_res < resultat:
                resultat = nouveau_res
                meilleur_coup = i
            del(copie)
